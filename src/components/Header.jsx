...
const Header = ({
  ...
  return (
    <header
      className="bg-gradient-to-tr from-red-600 to-red-500 text-white flex flex-col sm:flex-row items-center p-3 sm:p-4 shadow-md relative overflow-hidden"
      role="banner"
    >
      {/* Pokeball decorative elements */}
      <div className="absolute -right-16 -top-16 w-32 h-32 bg-white rounded-full opacity-10"></div>
      <div className="absolute -right-12 -top-12 w-24 h-24 bg-red-700 rounded-full opacity-10"></div>
      ...
      <div className="flex flex-col sm:flex-row items-center gap-3 sm:gap-6 z-10">
        <div
          className="flex gap-4 sm:gap-6 text-sm sm:text-base px-2 py-1 sm:px-4 bg-red-700/40 rounded-lg"
          role="status"
          aria-live="polite"
        >
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className={`bg-red-700 text-white px-3 py-1 rounded-lg text-sm sm:text-base cursor-pointer border border-red-400 focus:outline-none focus:ring-2 focus:ring-red-300 ${
